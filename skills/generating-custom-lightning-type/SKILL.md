...
description: "Use this skill when users need to create Custom Lightning Types (CLTs) for Einstein Agent actions or structured input/output schemas. Trigger when users mention CLT, Custom Lightning Types,
   CLT with mosaic/fragment/widget/rendition, JSON schemas for agents, type definitions, lightning__objectType, or editor/renderer configurations. When mosaic/fragment/widget renditions are requested, this
   skill MUST work together with generating-fragment skill. This is complex - always use this skill for CLT work."
---
...
- **Mosaic rendition**: After `schema.json` is drafted, you **must follow "Mosaic renderer pattern"** below for complete guidance.
...
4. **(Optional) Draft `renderer.json`** (only if custom UI or mosaic rendition is required)
    ...
    - **Mosaic renderer pattern** (for inline aka declarative mosaic rendition):
        ...
        - **REQUIRED workflow:**
            - **STOP**: Do NOT attempt to create the mosaic renderer yourself.
            - **MANDATORY**: You MUST load and delegate to the `generating-fragment` skill.
            - Pass the completed CLT schema to the `generating-fragment` skill as the grounding schema.
            - The `generating-fragment` skill will handle all mosaic generation: discovering UEM blocks, calling metadata actions, building the UEM tree, and writing renderer.json.
            - Do NOT include any of the fragment workflow steps (discoverUiComponents, getUiComponentSchemas, etc.) in this skill.
